...
安装
```shell
wget https://raw.githubusercontent.com/JiuBoy/frps/master/frps_linux_install.sh && chmod +x frps_linux_install.sh && ./frps_linux_install.sh
```
...
卸载
```shell
wget https://raw.githubusercontent.com/JiuBoy/frps/master/frps_linux_uninstall.sh && chmod +x frps_linux_uninstall.sh && ./frps_linux_uninstall.sh
```
...
`REPO="JiuBoy/frps"` 由于 **fork** 到你自己的仓库,需修改`JiuBoy`为你的GitHub账号ID.

#### 运行一键脚本
修改以下脚本链接中的`JiuBoy`为你的GitHub账号ID后,运行即可.
```shell
wget https://raw.githubusercontent.com/JiuBoy/frps/master/frps_linux_install.sh && chmod +x frps_linux_install.sh && ./frps_linux_install.sh
```
#### 卸载脚本
...
```shell
git clone https://github.com/JiuBoy/frps.git
# git clone 本仓库
